...
import { transformDocument, mergeSchemas } from './transformers.js';
...
/**
 * Quote a column/table name if it's a reserved word or contains special characters
 * @param {string} name 
 * @returns {string}
 */
...
export function exportToSql(collectionName, documents, onProgress = console.log) {
    ...
    if (config.sql.includeCreateTable) {
        sql += generateCreateTable(collectionName, mergedSchema);
    }

    // Add INSERT statements
    sql += `-- Data\n`;
    for (const doc of transformedDocs) {
        sql += generateInsert(collectionName, doc, columns) + '\n';
    }
    sql += 'GO\n';

    // Write file
    const filename = `${collectionName}.sql`;
