...
/**
 * Generate a short hash from a string (for truncated field names)
 * @param {string} str 
...
export function sanitizeFieldName(fieldName) {
    ...
    // Truncate long field names (SQL Server max is 128)
    // Keep 119 chars + underscore + 8-char hash = 128
    if (sanitized.length > 120) {
        const hash = shortHash(fieldName);
        sanitized = sanitized.substring(0, 119) + '_' + hash;
    }
...
/**
 * Extract parent document ID from a Firestore path
 * e.g., "users/abc123/orders/order-001" -> "abc123"
 * @param {string} path 
 * @returns {string|null} Parent ID or null if root collection
 */
export function extractParentId(path) {
    const parts = path.split('/');
    // Path format: collection/docId/subcollection/docId/...
    // For a subcollection doc, parent ID is at index -3
    if (parts.length >= 4) {
